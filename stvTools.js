...
(function () {
  ...
  var runInContext = function runInContext(context) {
    ...
    function getHolder(func) {
      var object = hasOwnProperty.call(stvTools, "placeholder")
        ? stvTools
        : func;
    ...
    function emailRegex(string) {
      ...
      const flag = regex.test(string);
      return flag;
    }

    /**
     * 密码正则：必须包含大小写字母、数字、特殊字符
     * @param {String} string
     */
     ...
     * @param {String} maxSize: 文件大小
     *
     */
    function fileRegex(file, type, maxSize) {
      const isBPMN = type === "BPMN";
      const nameArr = file.name.split(".");
      // type === BPMN
      const case1 = isBPMN && nameArr[1].toLowerCase() !== "bpmn";
      // type === image
      const case2 =
        !isBPMN &&
      ...
      if (file.size / (1024 * 1024) > maxSize) {
        return { flag: false, meg: `上传文件不超过${maxSize}M` };
      } else if (case1 || case2) {
        return { flag: false, meg: "请上传正确的文件格式" };
      } else {
        return { flag: true };
      }
    }
    ...
    function client() {
      ...
      system.x11 = p == "X11" || p.indexOf("Linux") == 0; //检测Windows操作系统
    ...
    stvTools.emailRegex = emailRegex;
    stvTools.passwordRegex = passwordRegex;
    stvTools.getNowFormatDate = getNowFormatDate;
    stvTools.fileRegex = fileRegex;
    stvTools.uniqueArr = uniqueArr;
    stvTools.client = client;
    stvTools.uniqueArr = uniqueArr;
    ...
    stvTools.prototype.toJSON =
      stvTools.prototype.valueOf =
      stvTools.prototype.value =
        wrapperValue;
